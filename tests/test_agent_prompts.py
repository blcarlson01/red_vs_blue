...
def main():
    ...
    print("-" * 70)
    obs = env.observe("p0")
    prompt = agent._system_prompt() + agent._role_prompt(obs) + agent._user_prompt(obs)
    ...
    if current_pres == agent.player_id:
        ...
        prompt = agent._system_prompt() + agent._role_prompt(obs_pres) + agent._user_prompt(obs_pres)
        if "nominate" in prompt.lower():
