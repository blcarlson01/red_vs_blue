...
def main():
    ...
    check("1a", len(env.player_ids) == 5, "Wrong player count")
    check("1b", env.round == 0, "Round should start at 0")
    check("1c", not env.done, "Game should not be done at start")
    check("1d", env.current_phase == "discussion", f"Expected 'discussion', got '{env.current_phase}'")
    print()

    # Test 2: Patch Deck
    print("Test 2: Patch Deck")
    env = RedvsBlueEnv(num_players=5, seed=42)
    check("2a", len(env.patch_deck) == 17, f"Wrong deck size: {len(env.patch_deck)}")
    check("2b", env.patch_deck.count("blue") == 6, f"Wrong blue count: {env.patch_deck.count('blue')}")
    check("2c", env.patch_deck.count("red") == 11, f"Wrong red count: {env.patch_deck.count('red')}")
    print()
    ...
    check("3a", roles.count("blue") == 3, f"Wrong blue count: {roles.count('blue')}")
    check("3b", roles.count("red") == 1, f"Wrong red count: {roles.count('red')}")
    check("3c", roles.count("apt_leader") == 1, f"Wrong apt_leader count: {roles.count('apt_leader')}")
    check("3d", env.true_apt_leader_id in env.player_ids, "APT Leader not in player list")
    print()
    ...
    check("4a", env.done and env.blues_win(), "Blue win at 6 not working")
    ...
    env._execute_legislative_session(0)
    check("4b", env.done and env.reds_win(), "Red win at 6 not working")
    ...
    env.end_round()
    check("4c", env.done and env.reds_win(), "APT Leader win not working")
    ...
    check("5a", env.current_ciso_idx == 2, f"Special election failed: idx={env.current_ciso_idx}")
    ...
    check("5b", 'p1' in env.investigation_results, "Investigation not recorded")
    ...
    check("5c", target in env.fired_players, "Fire not recorded")
    ...
    check("6a", env.current_phase == "discussion", "Start phase wrong")
    env.end_round()
    check("6b", env.current_phase == "nomination", "Discussion->Nomination failed")

    # Nomination -> Voting
    ciso = env.player_ids[env.current_ciso_idx]
    env.step(ciso, {'nomination': 'p1'})
    check("6c", env.current_phase == "voting", "Nomination->Voting failed")
    ...
    check("7a", env.done, f"Game should end: round={round_count}, max={max_attempts}")
    check("7b", round_count < max_attempts, f"Game took too long: {round_count} rounds")
    print()
